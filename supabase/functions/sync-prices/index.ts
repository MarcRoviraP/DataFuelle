  ...
  try {
    const response = await fetchWithRetry(API_URL, {
      headers: {
        ...
        "User-Agent": "datafuelle-sync/2.0"
      }
    });
    ...
    console.log(`📡 Received ${stationsList.length} stations. Processing deduplication...`);
    ...
    for (let i = 0; i < stationsList.length; i += CHUNK_SIZE) {
      ...
      // 1. Get latest prices for this batch to compare
      ...
      if (fetchError) console.error("Error fetching latest prices:", fetchError);

      // Build a map of latest recorded prices per station
      ...
      // 2. Prepare Station Metadata (Upsert)
      ...
      // 3. Prepare Price History (Insert only if changed)
      ...
      if (pricesToInsert.length > 0) {
        ...
        if (priceError) console.error(`Error in prices chunk [${i}]:`, priceError);
        else totalInserted += pricesToInsert.length;
      }

      totalProcessed += chunk.length;
      if (i % 2000 === 0) {
        console.log(`Progress: ${totalProcessed}/${stationsList.length} (Inserted: ${totalInserted}, Skipped: ${totalSkipped})`);
      }
    }

    const duration = ((Date.now() - startTime) / 1000).toFixed(2);
    console.log(`✅ Sync Completed in ${duration}s!`);
    console.log(`📊 Stats: Stations Upserted: ${stationsUpdated} | History Inserted: ${totalInserted} | History Skipped: ${totalSkipped}`);
    ...
    return new Response(JSON.stringify({ 
      ...
      stats: {
        ...
        inserted_prices: totalInserted,
        skipped_prices: totalSkipped,
